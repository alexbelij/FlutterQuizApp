...
class Collection<T> {
  ...
  Future<void> addQuiz(Map data) async {
    FirebaseUser user = await _auth.currentUser();
    Document<T> ref = Document(path:  'quizzes/' + Uuid().v4());
...
class UserData<T> {
  ...
  Future<void> upsert(Map data) async {
    ...
    .where("total", isGreaterThan: total).getDocuments().then((snapShot) {
      upUsers = snapShot.documents.length;

      data['ranking'] = upUsers + 1;
      Document<T> ref = Document(path:  '$collection/${user.uid}');
      ref.upsert(data);
    });
  }
